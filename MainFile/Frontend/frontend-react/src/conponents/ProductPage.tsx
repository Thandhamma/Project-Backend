...
const mockProducts: Product[] = [
    ...
    { id: 4, name: 'Care Bear Always There (M)', price: 325, imageUrl: '/ImageForProject/CareBearAlwaysThereBearPlushToyswithoutline-med.png', rating: 3.8, tags: [] },
    { id: 5, name: 'Sour Grumpy Yummy Yuppy', price: 500, imageUrl: '/ImageForProject/care-bears-sour-grumpy-bears-3-5oz-theater-box.jpg', rating: 5.0, tags: ['Limited', 'New arrival'] },
    { id: 6, name: 'Blue shirt with B&B', price: 945, imageUrl: '/ImageForProject/blue-shirtbear.jpg', rating: 2.5, tags: [] },
];


// ==================================================================
// ✅✅✅ ส่วนที่หายไปคือตรงนี้ครับ นำกลับมาวางไว้ที่เดิม ✅✅✅
// ==================================================================
    ...
    return (
        <div className="product-card">
            <a href="#" className="block product-card-inner border border-gray-200 rounded-lg p-3 bg-white transition-all transform duration-200 hover:shadow-lg hover:-translate-y-1">
                <img src={product.imageUrl} alt={product.name} className="w-full h-40 object-contain rounded-md mb-3" />
                <div className="flex items-center justify-between px-1">
                    <div className="text-sm text-gray-800 truncate">{product.name}</div>
                    <div className="text-sm font-semibold text-gray-900 whitespace-nowrap">{product.price} ฿</div>
                </div>
                ...
                <div className="flex items-start flex-wrap gap-2 h-5 mt-2">
                    {product.tags.map(tag => (
                        <span key={tag} className="px-2 py-0.5 text-xs text-pink-500 bg-pink-100 rounded-full">{tag}</span>
                    ))}
                </div>
            </a>
        </div>
...
function ProductPage({ onLoginClick }: ProductPageProps) {
    ...
    return (
        <>
            ...
            <main className="overflow-y-auto w-full px-4 py-1 bg-rose-50 flex-grow">
                <h3 className="comicRelief text-3xl font-extrabold mb-6 mt-3 text-center text-white bg-gradient-to-r from-pink-400 via-pink-500 to-purple-500 px-6 py-2.5 rounded-full shadow-[0_4px_20px_rgba(255,105,180,0.6)] w-full max-w-2xl mx-auto border border-white/40" style={{ textShadow: "2px 2px 6px rgba(0,0,0,0.35), 0 0 8px rgba(255,255,255,0.9);" }}>
                ...
                <div className="flex gap-4 p-2 h-[calc(100vh-200px)]">
                    ...
                    <FilterSidebar
                        ...
                        onToggle={() => setIsSidebarCollapsed(!isSidebarCollapsed)} // แก้เป็น Toggle เพื่อให้กดซ่อนได้ด้วย
